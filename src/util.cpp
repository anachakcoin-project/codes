...
const char * const BITCOIN_CONF_FILENAME = "anachakcoin.conf";
const char * const BITCOIN_PID_FILENAME = "anachakcoin.pid";
...
#else
    const char* pszModule = "anachakcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Anachakcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Anachakcoin";
#else
    // Unix
    return pathRet / ".anachakcoin";
#endif
#endif
}
